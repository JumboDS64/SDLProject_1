...
bool Map::is_solid(glm::vec3 position) {
    int tile_x = (floor(position.x/m_tile_size + 0.5)); //You need to divide by m_tile_size first, otherwise you're flooring/ceiling to the nearest 1 instead of the nearest m_tile_size, which doesn't work if m_tile_size is not an integer value.
    int tile_y = -(ceil(position.y/m_tile_size - 0.5));
...
float Map::round_to_corner_x(float x) {
    return (floor(x/m_tile_size + 0.5)) * m_tile_size;
}

float Map::round_to_corner_y(float y) {
    return (ceil(y/m_tile_size - 0.5)) * m_tile_size;
}
